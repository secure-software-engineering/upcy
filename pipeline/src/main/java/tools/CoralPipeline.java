...
public class CoralPipeline implements PipelineTool {
  ...
  public void runTool(
    ...
    if (jars.size() > 1) {
      List<Path> filteredJars = jars.stream()
          .filter(p -> !p.getFileName().toString().endsWith("-sources.jar") && !p.getFileName()
              .toString().endsWith("-javadoc.jar")).collect(
              Collectors.toList());
      String[] projectGav = projectNameClear.split("_");
      filteredJars = filteredJars.stream()
    ...
    if (exitCode == IOUtils.TIMEOUT_EXITCODE) {
      throw new BuildToolException(
          String.format(
              "The process timed out when executing '%s'.\nStdout: %s\nStderr: %s",
